...
import type { JobDefinition } from "../../../src/lib/jobRegistry/core.ts";
...
const index = await import("../../../src/lib/jobRegistry/index.ts");
...
test("startAll isolation: a missing handler skips that job, starts the rest", async () => {
  ...
  core.getDbInstance();
  const { upsertJob } = await import("../../../src/lib/db/jobRegistryDb.ts");
  upsertJob({
    id: "orphan",
    type: "interval",
    cron: null,
